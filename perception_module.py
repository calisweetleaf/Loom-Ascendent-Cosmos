...
class SensoryFilter:
    """
    Filters sensory input data to extract relevant features and reduce noise.
    """
    def __init__(self, filter_type: str = "low_pass", threshold: float = 0.5):
        """
        Initialize the sensory filter.

        Args:
            filter_type: Type of filter to apply (e.g., "low_pass", "high_pass").
    ...
    def apply_filter(self, data: list[float]) -> list[float]:
        ...
        Args:
            data: List of sensory input values.

        Returns:
            Filtered data as a list of values.
        """
        if self.filter_type == "low_pass":
            return [value if value <= self.threshold else self.threshold for value in data]
        elif self.filter_type == "high_pass":
            return [value if value >= self.threshold else 0 for value in data]
        else:
            raise ValueError(f"Unknown filter type: {self.filter_type}")

    def set_threshold(self, threshold: float):
        """
        Update the threshold value for the filter.
    ...
    def set_filter_type(self, filter_type: str):
        ...
        Args:
            filter_type: New filter type (e.g., "low_pass", "high_pass").
        """
        self.filter_type = filter_type